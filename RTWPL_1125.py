...
def sqlfromfile (filenam, crsr):
    wrkfl = open(filenam, 'r')  # executes queries in sql file
    sqlfl = wrkfl.read()
    wrkfl.close()
    sqlcmds = sqlfl.split(';')
    for cmd in sqlcmds:
...
ELSE (CASE WHEN LNCEL_FDD.dlRsBoost=1600 THEN 6 ELSE NULL END)END)END)END)END)END AS PowerBoost, LNCEL.pmax, LNCEL.phyCellId, 
...
b.TILT_ELECTRICO, b.TILT_MECANICO,b.TWIST,b.CELLID,b.GRUPO,b.LAC,b.TECNOLOGIA,b.ESTADO,b.FECHA_ESTADO,b.PROVEEDOR,b.REGIONALCLUSTER,
b.TIPO_AREA_COBERTURA,b.OWA, 
100*((powp(10,WCEL.PtxPrimaryCPICH/100.0)/1000)/(powp(10,WCEL.PtxCellMax/100.0)/1000)) AS CpichPwrRatio, 
WCEL.PtxPrimaryCPICH, WCEL.PtxCellMax, WCEL.MaxDLPowerCapability
...
sqlxtrtcerl3 = (rwtpsql1 + rwtpsql3 + 
" (w.Prefijo = r.Prefijo COLLATE NOCASE COLLATE NOCASE) " + umtssql4)  # query selection for RWTP LNCEL based 
lkpisql3 = """
...
sqlxtrtcl3 = (lkpisql1 + lkpisql3 +  
" (w.Prefijo = r.Prefijo COLLATE NOCASE) " + umtssql4)  # LTE kpi Prefijo based
...
UMTS_Offndr = Path('C:/sqlite/UMTS_Offender.sql')  # UMTS ofender queries
LTE_Offndr = Path('C:/sqlite/LTE_Offender.sql')  # LTE ofender queries
today = date.today()
exe = "C:\\7za\\7za.exe"
...
connd.create_function('powp',2, math.pow)  # create math functions not defined in sqlite3 library
connd.create_function('log10p', 1, math.log10)
conn.create_function('powp',2, math.pow)  # create math functions not defined in sqlite3 library
conn.create_function('log10p', 1, math.log10)
    ...
    if (opt2 != 1):  # UMTS or both
        if (opt != 2):  # UPDATE or both
            ...
            if opt1 != 2:  # kpi or both
                ...
                sqlfromfile (UMTS_Offndr, cur)  # executes queries in sql file
                tabs = ['Avail_Rural', 'Avail_Urban', 'Pwr_Fail_Rural', 'Pwr_Fail_Urban', 'Denied_HS_Rural',
                'Denied_HS_Urban', 'Acc_CS_Rural', 'Acc_CS_Urban', 'Drop_CS_Rural', 'Drop_CS_Urban', 'Prach_DeltaU', 
                'Prach_DeltaR', 'Prach_GT1500U', 'uweek']
                filen = 'UMTS_Offender'
                sqltabexport(conn, tabs, filen, dbtgt)  # saves sqlite tabs in filen  
    ...
    if(opt2 !=2):  # LTE or both
        ...
        if (opt != 2):  # update
            ...
            if opt1 != 2:  # KPI OR BOTH
                ...
                sqlfromfile (LTE_Offndr, curl)  # executes queries in sql file
                tabs = ['Avail_Rural', 'Avail_Urban', 'RACH_Rural', 'RACH_Urban', 'TPUT_Rural',
                'TPUT_Urban', 'PRB_Rural', 'PRB_Urban', 'UEdist_DeltaU', 'UEdist_DeltaR', 'UEdist_GT1000U']
                filen = 'LTE_Offender'
                sqltabexport(connl, tabs, filen, dbtgt)  # saves sqlite tabs in filen  
else:
    # proc = [11]
    proc = [1,6,7,8]
    for iter1 in proc:
        if iter1 == 1:
            tabs = ['rtwp7day', 'rtwp7dayl', 'rtwp7dayldif', 'rtwp7daylport']
            filen = 'RTWP_Audit'
