...
cur = conn.cursor()
sqlcsvimport(conn, cur, 'baseline_Site', 'Sitios')  # Bl to new daily dB 
sqlcsvimport(conn, cur, 'BaselineLTE', 'LTE')
RET_sql = Path('C:/sqlite/RET_Info_211005.sql')  # RET queries 
sqlfromfile (RET_sql, cur)  # run queries from sql file
tabs = ['RET_LTE']
filen = 'RET_Config'
sqltabexport(conn, tabs, filen, dbtgt)  # save db table to xlsx file 
cur.close()
conn.close()