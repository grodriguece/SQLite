...
" (t.LNBTSname = r.LNBTSname COLLATE NOCASE);")  # query selection for RWTP LNBTS based 
lkpisql3 = """
...
if opt != 4:  # dBAct, Getdata, Both 
    ...
    if (opt2 != 1):  # UMTS or both
        ...
        df.to_sql('wcel_param', connr, if_exists='replace', index=False, chunksize=10000)
        ...
        if (opt != 1):  # DATA XTRACT TO TEMPLATE CSV
            if opt1 != 1:  # RTWP or both 
                df3 = totemplate(connr, xlspath)  # info from sector_set csv to sql
                ...
                if xtrctyp1 == 0:
                    ...
                    totemplate1(connr, xlspath, xtrctyp1)
            if opt1 != 2:  # kpi or both
                ...
                if xtrctyp1 == 0:
                    ...
                    totemplate1(conn, xlspath1, xtrctyp1)  # END OF UMTS PROCESS
    if(opt2 !=2):  # LTE or both
        ...
        if (opt != 1):  # DATA XTRACT TO TEMPLATE CSV
            if opt1 != 1:  # RWTP or both 
                df3 = totemplate(connr, xlspath3)  # info from sector_set
                xtrctyp = list(df3.columns)  # column names from sector_set  
                xtrctyp1 = xtrctypelr(xtrctyp[0])  # query selection based on filter option
                if xtrctyp1 == 0:
                    print('No valid sector_set {repfil} filter type'.format(repfil=xtrctyp[0]))
                else:
                    totemplate1(connr, xlspath3, xtrctyp1)
            if opt1 != 2:  # kpi or both
                df3 = totemplate(connl, xlspath2)
                xtrctyp = list(df3.columns)
                xtrctyp1 = xtrctypel(xtrctyp[0])
                if xtrctyp1 == 0:
                    print('No valid sector_set {repfil} filter type'.format(repfil=xtrctyp[0]))
                else:
                    totemplate1(connl, xlspath2, xtrctyp1)  # END OF UMTS PROCESS
        if (opt != 2):  # update
            if opt1 != 1:  # RTWP or both 
